...
weight_decay = 0.0001
t_max = 5  # max number of cuts supported (hence max of 5^2 crops + 5 OOD = 30)
crop_size = 40  # size of each crop ("pixels")
max_crops = t_max**2 + t_max  # max possible crops in the directory
output_dim = t_max**2 + 2  # added 2 for OOD and zeros (padding) marking
LSTM_layer = 128


def plot_history(history, baseline=None):
    # learning history plotter
    his = history.history
    val_acc = his['val_acc']
    # train_acc = his['acc']
...
def define_model():
    ...
    model.add(Bidirectional(LSTM(LSTM_layer, return_sequences=True), merge_mode='sum'))
...
def parse_output(data, n_crops):
    ...
    t = np.floor(np.sqrt(n_crops))   # extract the shredding factor (2,3,4,5 etc.)
    n_OODs = int(n_crops - t**2)     # extract the number of OoDs in the sample set
    n_pic_crops = int(t**2)
    print('t = ' + str(t))
    ...
    # Second we softmax and clear OOD elements:
    ...
    # We now run Sinkhorns Softmax rows and cols (n iterations)
    for k in range(4):
...
def arrange_image(output, crops_set, t, pixels, size_wo_pad, n):
    # reconstruct figure from set of crops and location vector (predicted by network)
    t = int(t)
...
def main():
    ...
    x_doc, y_doc = data_doc.generate_data(tiles_per_dim=[2, 4, 5])
    x, y = data_pic.generate_data(tiles_per_dim=[2, 4, 5])

    x = np.append(x, x_doc, axis=0)
    y = np.append(y, y_doc, axis=0)
    ...
    history = Model.fit(x_train, y_train, callbacks=[checkpoint], epochs=20, verbose=1, batch_size=64, validation_data=(x_test, y_test))

    plot_history(history)

    Model.save('Recovery_pic_docs.h5')  # save network

    prediction = Model.predict(x_train[0:10, :, :, :, :])

    print(prediction.shape)

    # visual sanity check on output:
    ...
    for test in range(10):
        ...
        arrange_image(output, x_train[test, :, :, :, :],
                      t=np.floor(np.sqrt(crops)),
                      pixels=crop_size,
                      size_wo_pad=crops,
                      n=test)
